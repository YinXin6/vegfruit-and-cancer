...
library(epiDisplay)
library(openxlsx)
data <- fread('C:/group_jk226/mhcdc/yinx/final1016.csv')
#诊断日期在6.30之后
手动
#删除C码有死亡日期但无诊断日期的对象
data <- data %>% slice(
...
write.csv(data,'C:/group_jk226/mhcdc/yinx/final1017.csv',row.names = F)

data <- fread('C:/group_jk226/mhcdc/yinx/final1017.csv')
#病例数，随访人年数，发病密度每10w人年
library(purrr)
...
write.xlsx(IR,'C:/group_jk226/mhcdc/yinx/结果/IR女.csv')
...
cox_all <- cox_all %>% 
  ...
  dplyr::select(cancer,term,HR,lower,upper,HR_CI,p.value)
write.xlsx(cox_all,'C:/group_jk226/mhcdc/yinx/结果/森林图果蔬摄入男.xlsx')

##营养素四分位
data <- data %>% mutate(
  across(c(30:37,43:45,74:86),
         ~ntile(.x,4),
         .names = "{.col}A"))
         
##营养COX
for (i in names(data)[c(3:16,109,113:115,118:141)])
  data[[i]] <- factor(data[[i]])
cancer_cols <- c("肿瘤","肺癌","胃癌","结直肠癌","饮食相关肿瘤")
cox_results <- list()
for(cancer in cancer_cols){
  #构建生存对象
  surv_obj <- Surv(time = data$followupyears,event = data[[cancer]])
  cox_model <- coxph(surv_obj~Age+Sex+education+被动吸烟+alcohol+tea+活动水平
                     +肥胖+腰围+高血压+糖尿病+冠心病+脑卒中+高脂血症
                     +VAA+VB1A+VB2A+VB3A+VCA+VEA+铁A+锌A+硒A
                     +`D20-5A`+`D22-6A`+`D18-3A`+`D20-4A`+`D18-2A`,data=data,subset=(Sex==2))
  #提取整理结果
  cox_summary <- broom::tidy(cox_model) %>% 
    mutate(cancer=cancer)
  cox_results[[cancer]] <- cox_summary
...
cox_all <- bind_rows(cox_results)
cox_all <- cox_all %>% 
  filter(grepl("A",term)) %>% #只保留总摄入分类结果
  mutate(
    HR_CI=paste0(round(exp(estimate),2),"(",
                 round(exp(estimate-1.96*std.error),2),"-",
                 round(exp(estimate+1.96*std.error),2),")")
  ) %>% 
  dplyr::select(cancer,term,HR_CI,p.value)
write.xlsx(cox_all,'C:/group_jk226/mhcdc/yinx/结果/COX营养素女.xlsx')
##单因素
library(epiDisplay)
for (i in names(data)[c(3:16,109,113:115)])
  data[[i]] <- factor(data[[i]])
...
write.xlsx(table1,'C:/group_jk226/mhcdc/yinx/结果/单因素果蔬摄入.xlsx')
...
AIC(fit)##确定拐点3~5
an <- anova(fit)#摄入量与肿瘤是否存在非线性关系P<0.05
an
